...
9. The USB port is a **stepped bore** — overmold pocket (`CANOPY_USB_OM_*`) outside, shell neck (`CANOPY_USB_W` / `canopy_usb_z`) inside — because the jack mouth sits 4.41 mm behind the outer face and a straight hole gives 34% shell engagement while blocking the overmold. Probe each section at its own depth; a mid-wall Y lands inside the pocket. `CANOPY_RIDGE_TOP_Z` is partly derived from the pocket and must also clear `CANOPY_NORTH_ROUND_R` — `test_usb_pocket_stays_buried_under_the_north_shoulder`.
