...
14. The canopy roofs carry **puzzle strokes**: two straight lines drawn across the ASSEMBLED pair, each crossing BOTH canopies, so the four strokes are really two lines and the mark completes only when both halves are on the desk. Two properties follow and are both tested. (a) The halves **cannot** look alike — one line meets each canopy at a different angle because they are splayed, so "right must not mirror left" is arithmetic, not a table of tuned numbers (on the right one stroke runs nearly lengthwise, dx/dy ≈ −0.20, while its partner on the left runs across at +1.79). (b) Collinearity across the gap is **exact** (~1e-14 mm), because both strokes are cut from one line rather than aimed at each other; if that number ever becomes merely small, someone has replaced the derivation with transcription. `canopy_puzzle` owns the assembled-frame definition and is **fitted from the design sketch**, not invented — the sketch was measured by pixel classification + PCA + a Hough split, its stroke pairs matched in page space to 0.11°, and one splay plus two lines fit all four strokes to **0.166 mm max / 0.124 rms**. Fitting rather than transcribing was deliberate: the four measured strokes individually imply 48.6° of splay from one pair and 50.8° from the other, so transcribing would bake that 2° disagreement in permanently instead of surfacing it as a residual.
    ...
    **Terminals: three kinds, and each end is exactly one of them.** (i) **Off an edge** — west and north behave identically, and that is the design: the stroke is aimed a break's-worth PAST the wall, the cutter's floor stays flat (`_offset_roofline` covers the roof only), and the groove holds full depth to the facet's top line then fades as the facet drops from under it. Measured on the right half, both edges: 0.5 / 0.35 / 0.15 / 0.0 mm at the top line and 0.1 / 0.2 / 0.3 mm past it — they match because both facets are 2:1. The overshoot is not about cutting further; it removes the square end a stroke gets when it stops ON the line, which reads as a groove that gave up short of the edge. (ii) **East** — each half's UPPER stroke breaks THROUGH the east arris (`CANOPY_EAST_X + CANOPY_PUZZLE_EAST_BREAK`); sharp, no facet, genuinely notched. (iii) **Free** — a trimmed stroke's terminal stops in open roof.

    Judgements about where the north mark leaves the part are made at the WALL, via `puzzle_north_crossings`, never at the terminal: the stroke is tilted, so a break's-worth of overshoot also slides the endpoint 0.2 mm west per mm — a position the groove never occupies.
    ...
    **`PUZZLE_LINE_NUDGE` is the one number in `canopy_puzzle` that is NOT from the sketch.** Line A is translated 3.3 mm perpendicular to itself: as fitted it left the right roof 1.3 mm from the NW corner, inside the cluster of invariant 15. A translation changes no angle and no relationship between the halves, so collinearity survives it exactly (~1e-14) and both halves move together — but 3.3 mm is ~20× `PUZZLE_SKETCH_RESIDUAL`, so line A no longer lands where the drawing puts it. Kept as a separate term, never folded into `PUZZLE_LINES`, so "fitted from the sketch" stays literally true of everything else and this can be zeroed in one edit. The value is bounded on both sides, and neither bound is taste — all measured at the **wall crossing**: ≥ 2.94 for the whole groove width to leave through the *straight* north edge (the corner arc is tangent at `CANOPY_WEST_OUTER_X + CANOPY_CORNER_R`), ≤ 4.16 before the crossing crowds the USB pocket, and ≈ 4.6 before the **left** half's line 0 hits the north keep-out early enough to stop breaking the east arris — on the half nobody is watching while the right one is being tuned. At 3.3 the margins are 0.35 mm (corner) and 1.87 mm (pocket); the window's middle is 3.55. `canopy_puzzle_north_exit_window` states the window in the module that owns the keep-outs, the north break is refused outside it, and two tests pin both ends — including that zeroing the nudge puts the exit back in the corner, so the departure cannot survive as a no-op.
