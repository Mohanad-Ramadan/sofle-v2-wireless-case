...
def test_z_min_is_the_battery_jst_hung_under_the_board():
    """The underside JST, not the PCB seat, sets the hardware envelope's minimum Z."""
...
def test_encoder_is_included_by_default(side):
    """The EC11 and knob are part of the default hardware envelope."""
    ...
    assert abs(with_enc.max.Z - (K.knob_hem_z() + K.KNOB_H)) < 0.1, "knob crown sets max Z"
