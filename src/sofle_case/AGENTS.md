...
15. **The canopy's two top chamfers do not match, and the north one is the odd man out.** Measured on the bare right half: the **west** shoulder facet runs **1.2 mm** inboard and drops **2.4 mm** (top line x = 11.7) — the case rim's 2:1 drafted-facet style, which is what both are meant to be. The **north** top chamfer runs **2.4 mm** inboard and drops **1.2 mm** (top line y = **119.1**) — the legs inverted. Scan at x = 22: flat to y = 118.9, then 0.04 / 0.19 / 0.34 / 0.49 / 0.64 mm of drop at 119.2 / 119.5 / 119.8 / 120.1 / 120.4.

    Cause: `_yz_prism` hands OCC `chamfer(length=chamfer_v, length2=chamfer_h)` and its fallback loop tries `(l1,l2)`, then `(l2,l1)`, then the symmetric leg, accepting **whichever does not throw**. The assignment was never chosen, only survived — the failure mode `occ-chamfer-leg-order` warns about, where a wrong-but-successful slope looks like success. `_chamfer_west_top` is immune because it builds its facet as an explicit swept boolean and states which leg is which.

    Consequence to know before touching the NW corner: the two bands meeting there are **different widths in plan** (2.4 vs 1.2), so their mitre is lopsided before the corner round even crosses it. That asymmetry is a large part of why that corner reads crowded. `canopy_north_chamfer_run` is the single place that depends on the as-built run (it returns the VERTICAL leg, deliberately) — fixing the leg order is a one-word change there plus a deterministic chamfer in `_yz_prism`, and it would move the north top line to 120.3.
